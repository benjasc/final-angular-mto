...
export class MaintainerProfileComponent implements OnInit {
  ...
  constructor(public maintainerProfileService: MaintainerProfileService, public messageService: MessageService,
    public dialogService: DialogService, public globals: Globals, public logService: LogService
    , public router: Router, public areaService: AreaService) {
      this.pgCurrent = 1;
  ...
  ngOnInit() {
   ...
   console.log('Access : '  + JSON.stringify(this.menu) + ' / ' + this.usuario.idUser +  ' / '  + this.router.url);
    ...
    this.areaService.searchInfo("-1", "-1").subscribe(
      res => {
        this.listArea = this.utils.validarRespuestaFormatear(res);
  ...
  searchInfoUpdate(idProfile: string, area: String, perfil: String, idAreaProfile: String) {
    this.messageService.cargando(true);
   console.log('Search Update ' + ' idProfile ' + idProfile + ' area ' + area + ' perfil ' + perfil);
    ...
    this.myModalUpdateInputArea = idAreaProfile;
        ...
        this.maintainerProfileService.insertProfile(area, perfil).subscribe(
          resPerfil => {
            ...
            searchProfileArea + ' searchPerfil: ' + searchPerfil );
              this.maintainerProfileService.searchInfoProfile((searchIdProfile !== '' ? searchIdProfile : '-1'), (searchProfileArea !== '' ? searchProfileArea : '-1'),
