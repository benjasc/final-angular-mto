...
import { CompraNC } from './../../vo/compraNC';
import { DespachoNC } from './../../vo/despachoNC';
...
import { Compra } from '../../vo/compra';

@Component({
  selector: 'app-detalle-nc',
  templateUrl: './detalle-nc.component.html',
  styleUrls: ['./detalle-nc.component.scss']
})
export class DetalleNcComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit() {
    if (sessionStorage.getItem('numeroOC') === null || sessionStorage.getItem('numeroOC') === undefined) {
      ...
      this.obtenerDetalleCompra();
      this.obtenerDetalleDespacho();
  ...
  limpiarElementosSession() {
    ...
    sessionStorage.removeItem('codPais');
  }
  ...
  obtenerDetalleDespacho() {
   this.despachos = this.notaCreditoService.detalleDespachoNC(this.oc);
  }
  // obtenerDetalleDespacho() {
  //   this.notaCreditoService.detalleDespachoNC(this.oc).subscribe(
  //     response => {
  //       console.log(response);
  //       this.despacho = response.despacho;
  //       this.refrescarTable();
  //       this.numeroServiciosEnCarga = this.validarCarga(this.numeroServiciosEnCarga, 4, false);
  //     },
  //     err => {
  ...
  obtenerDetalleCompra() {
    this.comprasNC = this.notaCreditoService.detalleCompraNC(this.oc);
    this.compras = this.notaCreditoService.detalleCompra(this.oc);
  }
  // obtenerDetalleCompra() {
  //   this.notaCreditoService.detalleCompraNC(this.oc).subscribe(
  //     response => {
  //       console.log(response);
